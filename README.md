...
└── sp500_data.csv                  # exported S&P 500 

## Methodology
1. CPI Date Alignment
...
```bash
git clone https://github.com/NganTran-185/cpi-volatility-analysis.git
cd cpi-volatility-analysis
pip install yfinance pandas-datareader scikit-learn arch shap matplotlib seaborn
jupyter notebook Stock_Market_Reaction_to_Macroeconomic_Announcement.ipynb
```

Standard data science libraries (`pandas`, `numpy`, `scipy`) are assumed to be installed — they come with Anaconda or any standard Python data environment. An internet connection is required on first run to fetch market data via `yfinance` and CPI data via `pandas-datareader`.

Run all cells sequentially. An internet connection is required on first run to fetch market data via `yfinance` and CPI data via `pandas-datareader`.
